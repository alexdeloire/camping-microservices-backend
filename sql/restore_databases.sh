...
for DB_CONTAINER in "${!DATABASES[@]}"; do
  ...
  docker exec "$DB_CONTAINER" sh -c "PGPASSWORD='$DB_PASS' psql -U $DB_USER -d postgres -c 'DROP DATABASE IF EXISTS $DB_NAME;'"
  docker exec "$DB_CONTAINER" sh -c "PGPASSWORD='$DB_PASS' psql -U $DB_USER -d postgres -c 'CREATE DATABASE $DB_NAME;'"
